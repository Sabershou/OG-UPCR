...
class Matcher():
    ...
    def SC2_PCR(self, src_keypts, tgt_keypts, overlap=None):
        """
        Input:
            - src_keypts: [bs, num_corr, 3]
            - tgt_keypts: [bs, num_corr, 3]
        ...
        if overlap is None:
            ...
            k = 0.2
            # k = self.ratio * 0.1
        seeds = self.pick_seeds(src_dist, confidence, R=self.nms_radius, max_num=int(num_corr * k))
