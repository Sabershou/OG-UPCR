...
from .video_dataset import VideoDataset, VideoDataset_hybrid, VideoDataset_geo, VideoDataset_test
...
def get_dataset(cfg, split, overfit=None):
    ...
    if cfg.name == "ScanNet":
        ...
        root_path = cfg.RGBD_3D_ROOT
        dict_path = os.path.join(PROJECT_ROOT, f"make_dataset/3dmatch_{split}.pkl")
    elif cfg.name == "ScanNet_1500":
        root_path = cfg.SCANNET_1500_ROOT
        dict_path = os.path.join(PROJECT_ROOT, f"make_dataset/scannet_1500_{split}.pkl")
    else:
        raise ValueError("Dataset name {} not recognized.".format(cfg.name))
    ...
    elif cfg.input_type == 'test':
        dataset = VideoDataset_test(cfg, root_path, data_dict, split)
...
def collate_fn_hybrid(list_data, config, neighborhood_limits):
    ...
    for block_i, block in enumerate(config.architectures):
        ...
        if layer_blocks:
            ...
            p2i_list.append(batch_neighbors_kpconv(batched_points_list_img[layer], batched_points,
                                                   batched_lengths_list_img[layer], batched_lengths, r,
                                                   config.num_p2i).long())
            i2p_list.append(batch_neighbors_kpconv(batched_points, batched_points_list_img[layer],
                                                   batched_lengths, batched_lengths_list_img[layer], r,
                                                   config.num_i2p).long())
    ...
    dict_inputs = {
        ...
        'K': batched_K_list,
        'uid': batched_uid,
        'scene_name': batch_i['sequence_id'],
        'src_path': batch_i['path_0'],
        'tgt_path': batch_i['path_1'],
    }
