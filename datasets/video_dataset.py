...
class VideoDataset_geo(AbstractDataset):
    ...
    def __getitem__(self, index):
        ...
        src_pcd, tgt_pcd = self.generate_pointclouds(output["K"].unsqueeze(0),
                                                     [output['depth_' + str(i)].unsqueeze(0) for i in range(2)])
...
class VideoDataset_hybrid(AbstractDataset):
    ...
    def __init__(self, cfg, root_path, data_dict, split):
        ...
        self.rgb_transform_ssim = transforms.Compose(
            [
                transforms.Resize(self.image_dim),
                transforms.CenterCrop(self.image_dim),
                transforms.ToTensor(),
    ...
    def __getitem__(self, index):
        ...
        for i, id_i in enumerate(f_ids):
            ...
            # transform and save rgb
            rgb_gt = self.rgb_transform(rgb)
            rgb_ssim = self.rgb_transform_ssim(rgb)
            output["path_" + str(i)] = s_instance[id_i]["rgb_path"]
            output["rgb_" + str(i)] = rgb_gt
            output["rgb_ssim_" + str(i)] = rgb_ssim
            ...
            if self.processed:
                ...
                if self.name == 'RGBD_3DMatch':
                    dep = self.get_img(
                        os.path.join(self.root, s_instance[id_i]["dep_path"].replace('.depth', '.color')))
                else:
                    dep = self.get_img(
                        os.path.join(self.root, s_instance[id_i]["dep_path"].replace('.png', '_complete.png')))
                dep[dep > 8000] = 0
            else:
                dep = self.get_img(s_instance[id_i]["dep_path"])
                cam_scale = 1000
            ...
            # Extract P from extrinsics; we can about P between the frame pair
            P = s_instance[id_i]["extrinsic"]
            output["gt_pose_" + str(i)] = torch.from_numpy(P)
            P = torch.tensor(np.linalg.inv(P) @ P_rel).float()
            output["Rt_" + str(i)] = P
            # output["Rt_" + str(i)] = P[:3, :]
        ...
        src_pcd_list, tgt_pcd_list = self.generate_pointclouds(output["K"].unsqueeze(0),
                                                               [output['depth_' + str(i)].unsqueeze(0) for i in
                                                                range(2)])
...
class VideoDataset_test(AbstractDataset):
    """
        Dataset for video frames. It samples tuples of consecutive frames
    """
    ...
    def __init__(self, cfg, root_path, data_dict, split):
        name = cfg.name
        super(VideoDataset_test, self).__init__(name, split, root_path)
        self.cfg = cfg
        self.split = split
        self.use_padding = cfg.use_padding
        self.num_downsample = cfg.num_downsample
        ...
        self.processed = cfg.processed
        self.data_dict = data_dict
        self.test_npz_path = root_path
        self.test_list_path = os.path.join(root_path, 'test.npz')
        self.test_data_path = os.path.join(root_path, 'test')
        self.min_overlap = 0.4
        self.test_pose_root = None
        self.rgb_transform = transforms.Compose(
            [
                transforms.Resize(self.image_dim),
                transforms.CenterCrop(self.image_dim),
        ...
        # non-strided:  (0, 10), ( 1, 11), ( 2, 12), etc
        strided = split in ["valid", "test"]
        with np.load(self.test_list_path) as data:
            self.data_names = data['name']
        self.instances = self.dict_to_instances(self.data_dict, strided, self.data_names)
    ...
    def __getitem__(self, index):
        ...
        for i, id_i in enumerate(f_ids):
            ...
            if self.processed:
                ...
                if self.name == 'RGBD_3DMatch':
                    dep = self.get_img(
                        os.path.join(self.root, s_instance[id_i]["dep_path"].replace('.depth', '.complete')))
                else:
                    dep = self.get_img(
                        os.path.join(self.root, s_instance[id_i]["dep_path"].replace('.png', '_complete.png')))
                dep[dep > 8000] = 0
    ...
    def dict_to_instances(self, data_dict, strided, list_name):
        """
        converts the data dictionary into a list of instances
        Input: data_dict -- sturcture  <classes>/<models>/<instances>
        ...
        for cls_id in data_dict:
            for s_id in data_dict[cls_id]:
                ...
                num_frames = len(frames)
                n = int(s_id[-6:-3])
                correspondences = list_name[list_name[:, 0] == n][:, -2:]
                for i in correspondences:
                    instances.append([cls_id, s_id, tuple(i)])
                # for i in range(num_frames - self.num_views * stride):
                #     f_ids = []
                #     for v in range(self.num_views):
                #         f_ids.append(frames[i + v * stride])
